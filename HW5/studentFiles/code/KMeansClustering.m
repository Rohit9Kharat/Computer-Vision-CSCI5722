...
% Homework 5: KMeansClustering Function
...
%           point X(i, :) has been assigned to cluster c.
    ...
    if ~exist('centers', 'var')
        ...
        % Randperm function for initializing centers
    ...
    while true        
        ...
        % Intialize variables for sum and size of clusters
        sumClust = zeros(k, n);
        sizeClust = zeros(k);
        ...
        for i = 1:m
            ...
            % pdist2 function for computing euclidean distance
            euclDist = pdist2(samplePoint, centers); 
            
            % Minimum distance and its index
            [~, minIndex] = min(euclDist);
            
            % Assign point to center with smallest distance
            idx(i) = minIndex;
            
            % Assign sum and size of cluster
            sumClust(minIndex, :) = sumClust(minIndex, :) + X(i, :);
            sizeClust(minIndex) = sizeClust(minIndex) + 1; 
        end
        ...
        for j = 1:k
            sumClust(j, :) = sumClust(j, :) / sizeClust(j);
            centers(j, :) = sumClust(j, :);
        end
