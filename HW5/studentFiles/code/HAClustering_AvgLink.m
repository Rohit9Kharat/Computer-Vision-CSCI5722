...
% Homework 5: Task 3: Average Link HAC
% Instructor: Prof. Ioana Fleming
%--------------------------------------------%

function idx = HAClustering_AvgLink(x, k, visualize2D)
...
while numCluster > k
    ...
    % Find the closest pair of clusters and setting indices 
    ...
    for index =1:numPoints
        if index~=i 
            temp = mean([dists(j,index),dists(i,index)]);
             dists(i,index) = temp;
             dists(index,i) = temp;
...
% Now we need t reindex the clusters from 1 to k
