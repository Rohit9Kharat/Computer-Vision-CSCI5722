...
% Homework 5: HAClustering Function
...
%           point X(i, :) has been assigned to cluster c.
    ...
    while num_clusters > k
        ...
        % Finding row and col values of minimum distance value
        [row, col] = find(dists == min(dists(:)));
        i = row(1);
        j = col(1);
        ...
        % Assigning all points in cluster j to cluster i
        idx_j = j == idx;
        idx(idx_j) = i;
        ...
        idx_mean = [find(i == idx), find(j == idx)];
        centroids(i,:) = mean(X(idx_mean,:), 1);
        centroids(j,:) = Inf;
        ...
        % Squareform function returns a symmetric matrix for pairwise
        % distance
        dists = squareform(pdist(centroids));
        dists = dists + diag(Inf(m, 1));
