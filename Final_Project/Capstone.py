...
while True:
    ...
    #gray = cv2.filter2D(gray, -1, 0.5)
    #gray = cv2.blur(gray, ksize=(101, 101))
    #gray = cv2.medianBlur(gray, 79)
    #gray = cv2.bilateralFilter(gray, 63, 75, 75)

    #gray = cv2.erode(gray, 5, iterations=4)
    #gray = cv2.morphologyEx(gray, cv2.MORPH_OPEN, 5)
